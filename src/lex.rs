...
impl Debug for Token {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.write_fmt(format_args!("{:?}:{}", self.kind, self.intern))
    }
}
...
impl<'a> Lexer<'a> {
    ...
    fn push(&mut self, kind: TokenKind) {
        let intern = self.cart.interner.borrow_mut().intern(self.token_text());
        self.token_start = self.source_index;
        let bytes = take(&mut self.cart.bytes);
        self.cart.bytes = to_extend(&Token::new(kind, intern), bytes).unwrap();
        // dbg!(self.cart.bytes.len());
    }
