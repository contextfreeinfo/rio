...
impl<'a> Extractor<'a> {
    ...
    pub fn update_defs(&mut self) {
        self.update_defs_at(TreeBuilder::top_of(&self.cart.bytes), 0);
        // dbg!(&self.cart.defs);
    }
    ...
    pub fn update_tops(&mut self) {
        ...
        while let Some(kid) = stepper.next(&self.cart.bytes) {
            let Some(uid) = Node::uid(&self.cart.bytes, kid) else {
                continue;
            };
    ...
    fn read(&self, idx: usize) -> Node {
        Node::read(&self.cart.bytes, idx).0
    }

    fn top(&self) -> Node {
        self.read(TreeBuilder::top_of(&self.cart.bytes))
    }
        ...
        match node {
            Node::Block(block) => {
                let kids = self.wrap(|s| {
                    ...
                    while let Some(kid) = stepper.next(&s.cart.bytes) {
            ...
            Node::Call(call) => {
                ...
                let args = self.wrap(|s| {
                    ...
                    while let Some(arg) = stepper.next(&s.cart.bytes) {
            ...
            Node::Def(def) => {
                ...
                let typ = self.wrap_one(|s| s.convert_def_ids_at(s.read(def.typ), false));
            ...
            Node::Fun(fun) => {
                let params = self.wrap(|s| {
                    ...
                    while let Some(param) = stepper.next(&s.cart.bytes) {
            ...
            Node::Structured(structured) => {
                let defs = self.wrap(|s| {
                    ...
                    while let Some(def) = stepper.next(&s.cart.bytes) {
    ...
    pub fn update_defs_at(&mut self, idx: usize, def: usize) {
        let update_defs_at_range = |s: &mut Self, range: SizeRange| {
            ...
            while let Some((_, idx)) = stepper.next_idx(&s.cart.bytes) {
