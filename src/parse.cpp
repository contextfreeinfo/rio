...
auto parse_atom(ParseState* state) -> Node& {
  ...
  switch (tokens->kind) {
    ...
    case Token::Kind::String: {
      if (state->verbose) printf("string: %s\n", token_text(*tokens));
    ...
    default: {
      if (state->verbose) printf("junk: %s\n", token_name(*tokens));
      advance_token(state);
...
auto parse_block(ParseState* state, Token::Kind end) -> Node& {
  ...
  if (state->verbose) printf("begin block\n");
...
auto parse_call(ParseState* state) -> Node& {
  ...
  if (state->tokens->kind == Token::Kind::RoundL) {
    ...
    if (state->verbose) printf("begin call\n");
    node.Call.args = &parse_tuple(state);
    if (state->verbose) printf("end call\n");
    return node;
  } else {
    return callee;
  }
...
auto parse_fun(ParseState* state) -> Node& {
  ...
  if (state->tokens->kind == Token::Kind::RoundL) {
    parse_tuple(state);
    if (state->verbose) printf("args\n");
  }
  node.Fun.expr = &parse_expr(state);
  if (state->verbose) printf("end fun\n");
  return node;
}
...
auto parse_tuple(ParseState* state) -> Node& {
  ...
  if (state->verbose) printf("begin tuple\n");
  advance_token(state);
  auto buf_len_old = state->node_buf.len;
  ...
  advance_token(state);
  if (state->verbose) printf("end tuple\n");
  return node;
}
